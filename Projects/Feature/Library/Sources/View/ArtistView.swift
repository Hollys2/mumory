...
public func requestArtistSongs(artist: Artist, originalSongs: Binding<[Song]>) async{
    await withTaskGroup(of: [Song].self) { taskGroup in
        ...
        if albums.count > 1 {
            ...
            for await songs in taskGroup {
                originalSongs.wrappedValue.append(contentsOf: songs)
            }
            ...
            for track in tracks {
                guard let song = await fetchSong(songID: track.id.rawValue) else {print("no song");continue}
                originalSongs.wrappedValue.append(song)
            }
        }
        
        return originalSongs.wrappedValue
    }
}

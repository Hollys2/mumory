...
struct SocialFriendView: View {
        ...
        ZStack(alignment: .top) {
            ...
            VStack(alignment: .leading, spacing: 0, content: {
                ...
                if itemSelection == 0 {
                    ...
                    if currentUserViewModel.friendViewModel.recievedRequests.isEmpty {
                        VStack(spacing: 16) {
                        ...
                        .onAppear {
                            currentUserViewModel.friendViewModel.recievedNewFriends = false
                        }
                    }
                }
    ...
    private func searchFriend(id: String){
        status = .loading
        guard id != currentUserViewModel.user.id else {
            status = .noResult
            return
        }
        Task {
            ...
            if blockFriends.contains(currentUserViewModel.user.uId){
                status = .noResult
                return
            }
            ...
            if currentUserViewModel.friendViewModel.friends.contains(where: {$0.uId == friendUID}) {
                status = .alreadyFriend
            }else if currentUserViewModel.friendViewModel.recievedRequests.contains(where: {$0.uId == friendUID}) {
                status = .alreadyRecieve
            }else if currentUserViewModel.friendViewModel.friendRequests.contains(where: {$0.uId == friendUID}) {
...
struct RecievedRequestItem: View {
    ...
    private func deleteRequest() {
        ...
        Task {
            ...
            currentUserViewModel.friendViewModel.recievedRequests.removeAll(where: {$0.uId == friend.uId})
            status = .normal
        }
    }
    ...
    private func acceptRequest(){
        ...
        Task {
            ...
            currentUserViewModel.friendViewModel.recievedRequests.removeAll(where: {$0.uId == friend.uId})
            currentUserViewModel.friendViewModel.friends.append(friend)
            status = .normal
        }
    }
}
